...
for (const [day, exercises] of Object.entries(exercisesByDay)) {
    ...
    dayDiv.innerHTML = `<div class="day-header"><span>🏋 ${dayNames[day] ?? day}</span><span>Exercises</span></div>`;
            ...
            muscleBlock += `
                <div class="exercise-card">
                    ...
                    <div class="exercise-image">
                        ${e.ImageUrl ? `<a href="${e.VideoUrl ?? '#'}" target="_blank">
                                       ...
                                       </a>` : `<span class="text-muted p-3">No Image</span>`}
                    </div>
                </div>`;
        });
...
for (const [day, foods] of Object.entries(foodsByDay)) {
    const dayDiv = document.createElement('div');
    dayDiv.className = 'custom-card mb-4';
    dayDiv.innerHTML = `<div class="day-header"><span>🥗 ${dayNames[day] ?? day}</span><span>Nutrition Plan</span></div>`;

    const foodsByMeal = groupBy(foods, f => f.MealNumber);
    for (const [mealNumber, mealFoods] of Object.entries(foodsByMeal)) {
        let mealHtml = `<div class="meal-block"><h5>Meal ${mealNumber}</h5>
                        <table class="table-custom mb-3"><thead>
                        <tr><th>Food</th><th>Quantity (g)</th><th>Servings</th><th>Notes</th></tr></thead><tbody>`;
        mealFoods.forEach(f => {
            mealHtml += `<tr><td>${f.FoodName ?? ''}</td><td>${f.Quantity ?? ''} g</td>
                         <td>${f.NumberOfServings ?? ''} serving(s)</td><td>${f.Notes ?? ''}</td></tr>`;
        });
